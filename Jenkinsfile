        ...
        gitlabBuilds(builds: ['Build Jar', 'Add git tag', 'SonarQube analysis', 'Uploading build to S3', 'Deploying']) {
            ...
            if ("${env.BRANCH_NAME}" == "integration") {
                ...
                gitlabCommitStatus('Deploying') {
                    ...
                    try {
                        sshagent(['ssh-key']) {
                            sh "ANSIBLE_HOST_KEY_CHECKING=False ansible-playbook -l mpay-int-front -i hosts/hosts-int --skip-tags postfix,logrotate,nginx -t configuration,deploy -u market-pay-api -e \"{'ansible_become': false, 'version_to_deploy_api':'${pom.version}'}\" mpay_install.yml"
                        }
