            ...
            stage ('Uploading build to S3') {
                if (shouldDeploy(env) || shouldTag(pom, env)) {
                    withAWS (
                        ...
                        dir('target') {
                            s3Upload(file: "basejump-${pom.version}.jar", bucket: "delivery.steamulo.org", path: "steamulo/java-starter/java-starter-starter-${pom.version}-${env.BRANCH_NAME}.jar", metadatas:["sha1-checksum:${checksum}"])
                        }
                    }
                }
            }
